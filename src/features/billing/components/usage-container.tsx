  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">Generate speech starting at $0.30 per 1,000 characters</p>
      </div>
      <Button variant="outline" className="w-full text-xs" size="sm" disabled={isCheckoutPending} onClick={checkout}>
        {isCheckoutPending ? (
          <>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="text-xl font-bold tracking-tight text-foreground mt-1">{formatCurrency(estimatedCostCents)}</p>
...
export function UsageContainer() {
  ...
  return (
    <div className="group-data-[collapsible=icon]:hidden bg-background border border-border rounded-lg p-3">
      {data?.hasActiveSubscription ? <UsageCard estimatedCostCents={data.estimatedCostCents} /> : <UpgradeCard />}
    </div>
  )
}
