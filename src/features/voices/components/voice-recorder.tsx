...
export function VoiceRecorder({
  ...
  const { isRecording, elapsedTime, audioBlob, containerRef, error, startRecording, stopRecording, resetRecording } =
    useAudioRecorder()
  ...
  if (file) {
    return (
      <div className="flex items-center gap-3 rounded-xl border p-4">
        ...
        <Button type="button" variant="ghost" size="icon-sm" onClick={handleReRecord} title="Remove">
          <X className="size-4" />
        </Button>
      </div>
    )
